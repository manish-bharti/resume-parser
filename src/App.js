...
import ShowAnalysis from "./component/ShowAnalysis";
...
class App extends React.Component {
  ...
  async doOCR(file) {
    ...
    // parse the data
    const { data } = await this.worker.recognize(file.file);
    this.setState({
      isProcessing: false,
      ocrText: data.text,
      lines: data.lines,
            ...
            <div class="card-body">
              ...
              <div>
                {/* {this.state.lines.map((line) => {
                  const { x0: left, y0: top } = line.bbox;
                  const style = {
                    position: "absolute",
                    top: `${top}px`,
                    left: `${left}px`,
                ...
                {this.props.validFields.showResult && (
                  <ShowAnalysis requiredFields={this.props.validFields} />
                )}
              </div>
            </div>
          </div>
